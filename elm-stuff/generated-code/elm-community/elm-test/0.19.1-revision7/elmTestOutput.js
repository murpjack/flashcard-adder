...
var $author$project$Card$Back = {$: 'Back'};
var $author$project$Card$Front = {$: 'Front'};
...
var $author$project$Card$byId = F2(
	function (selectedId, cards) {
		return A2(
			$elm_community$list_extra$List$Extra$find,
			function (card) {
				return _Utils_eq(card.id, selectedId);
			},
			cards);
	});
...
var $author$project$Card$Id = function (a) {
	return {$: 'Id', a: a};
};
var $author$project$Card$idFromString = function (str) {
...
var $author$project$Card$delete = F2(
	function (card, cards) {
		return A2($elm_community$list_extra$List$Extra$remove, card, cards);
	});
...
var $author$project$Tests$dummy1 = A3(
	$elm$core$Basics$composeR,
	A2($author$project$Card$changePart, $author$project$Card$Back, 'Back'),
	A2(
		$elm$core$Basics$composeR,
...
var $author$project$Card$updateAtId = function (updated) {
	return A2(
		$elm$core$List$foldr,
		F2(
			function (card, acc) {
				return _Utils_eq(card.id, updated.id) ? A2($elm$core$List$cons, updated, acc) : A2($elm$core$List$cons, card, acc);
			}),
		_List_Nil);
};
...
var $author$project$Tests$all = A2(
	...
	_List_fromArray(
		[
			...
			'A new card should be created in list.',
			function (_v0) {
				var newCard = A3(
					$elm$core$Basics$composeR,
					A2($author$project$Card$changePart, $author$project$Card$Back, 'My back gives me trouble.'),
					A2(
						$elm$core$Basics$composeR,
					...
					_List_fromArray(
						[
							...
							return A2(
								...
								$elm$core$List$length(initialCards) + 1,
			...
			A2(
			$elm_explorations$test$Test$test,
			'A card should should be editable given some details and a card id.',
			function (_v1) {
				var initialCards = _List_fromArray(
					[$author$project$Tests$dummy1, $author$project$Tests$dummy2]);
				...
				return A2(
					...
					_List_fromArray(
						[
							...
							return A2(
								$elm_explorations$test$Expect$equal,
								$elm$core$List$length(initialCards),
								$elm$core$List$length(updatedList));
						},
							function (updatedList) {
							return A2(
								$elm_explorations$test$Expect$equal,
								editedFromList(updatedList),
			...
			A2(
			$elm_explorations$test$Test$test,
			'A card should be deleted from cards list.',
			function (_v2) {
				var toDelete = $author$project$Tests$dummy1;
				var initialCards = _List_fromArray(
					[$author$project$Tests$dummy1, $author$project$Tests$dummy2]);
				return A2(
					$elm_explorations$test$Expect$all,
					...
					A2($author$project$Card$delete, toDelete, initialCards));
			})
		]));
...
var $author$project$Test$Generated$Main$main = A2(
	...
	_List_fromArray(
		[
			...
			_List_fromArray(
				[
					$author$project$Test$Runner$Node$check($author$project$Tests$dummy1),
					$author$project$Test$Runner$Node$check($author$project$Tests$dummy2),
					$author$project$Test$Runner$Node$check($author$project$Tests$all)
				]))
		]));
